...
APIServerConnection.prototype.sendResponse = function(status, expires, lastModified, data, type) {
  ...
  if (expires < now + this.server.config.minTTL) {
    expires = now + this.server.config.minTTL;
  }
  if (expires > now + this.server.config.maxTTL) {
    expires = now + this.server.config.maxTTL;
  }
  ...
  if (lastModified < this.serverStartTime) {
    lastModified = this.serverStartTime;
  }
